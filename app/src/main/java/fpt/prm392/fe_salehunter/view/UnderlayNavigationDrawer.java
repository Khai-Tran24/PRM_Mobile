package fpt.prm392.fe_salehunter.view;
...
public class UnderlayNavigationDrawer {
    ...
    public void detectTouch(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (rtl)
                closeHandleRange = touchX < screenWidth - (overLayView.getX() + (overLayView.getWidth() * 80 / 100f)) && touchX >= screenWidth - (overLayView.getX() + overLayView.getWidth());
            else
                closeHandleRange = touchX < overLayView.getX() + (overLayView.getWidth() * 20 / 100f) && touchX >= overLayView.getX();
