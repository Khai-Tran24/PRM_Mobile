...
public class StorePageDataHelper {
    ...
    /**
     * Load more products for pagination using product search API
     */
    public static void loadMoreProducts(Context context, LifecycleOwner lifecycleOwner,
        ...
        viewModel.getNextPage().observe(lifecycleOwner, response -> {
            ...
            viewModel.removeObserverProductsData(lifecycleOwner);
        });
    }

    /**
     * Set product as favourite/unfavourite
     */
    public static void setFavourite(Context context, long productId, boolean favChecked, LifecycleOwner lifecycleOwner) {
        // Get the repository instance
        Repository repository = new Repository();

        // Get user token
        String userToken = UserAccountManager.getToken(context, UserAccountManager.TOKEN_TYPE_BEARER);
        ...
        LiveData<Response<BaseResponseModel<Object>>> favResponse;
        if (favChecked) {
            favResponse = repository.addFavourite(userToken, productId);
        } else {
            favResponse = repository.removeFavourite(userToken, productId);
        }
        
        // Observe the response
        favResponse.observe(lifecycleOwner, response -> {
            if (response != null && response.isSuccessful() && response.body() != null) {
                BaseResponseModel responseBody = response.body();
                if (responseBody.isSuccess()) {
                    String message = favChecked ? "Added to favorites" : "Removed from favorites";
                    Toast.makeText(context, message, Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(context, "Error: " + responseBody.getMessage(), Toast.LENGTH_SHORT).show();
                }
            } else {
    ...
    /**
     * Render initial products using search API to the adapter
     */
    public static void renderInitialProducts(Context context, LifecycleOwner lifecycleOwner,
                                             StorePageViewModel viewModel, ProductsCardAdapter adapter, 
                                             FragmentStorePageBinding binding, boolean[] endOfProducts) {

        viewModel.getStoreProducts().observe(lifecycleOwner, response -> {
            switch (response.code()) {
                case BaseResponseModel.SUCCESSFUL_OPERATION:
                    if (response.body() != null && response.body().getData() != null) {
                        ...
                        if (products.isEmpty()) {
                            ...
                            binding.storePageNoProducts.setVisibility(View.GONE);
                        }
                    } else {
                        endOfProducts[0] = true;
                        binding.storePageNoProducts.setVisibility(View.VISIBLE);
    ...
    /**
     * Interface for products loading callback
     */
    public interface ProductsCallback {
        void onProductsLoaded();
        void onProductsLoadError(String message);
    }
}
